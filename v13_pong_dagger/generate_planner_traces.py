...
def act(state, ob_curr, ob_prev, prev_action):
  return act_simple(state, ob_curr, ob_prev, prev_action)
#  obs, reward, done, info = env.step(0)

  prev_ball = get_ball(ob_prev) if ob_prev != None else None
  curr_ball = get_ball(ob_curr) if ob_curr != None else None
  
  # assert perfection 
#   if curr_ball != None:
#     assert curr_ball[0] < 75
  paddle = get_our_paddle(ob_curr) if ob_curr != None else None

  if (prev_ball is None or curr_ball is None or paddle is None):
    return 2

  if prev_ball[0] > curr_ball[0] or curr_ball[0] < 60 or curr_ball[0] > 71: 
    target = 40
  else:
...
def act_simple(state, ob_curr, ob_prev, prev_action):
